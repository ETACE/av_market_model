using Distributed, DataStructures, StatsPlots, Serialization, ArgParse, DataFrames
...
# Plotting Combined
exp_names = sort(collect(keys(experiments)))
	...
	for col in propertynames(aggregated_data[exp_names[1]][agg])
		if col != :step
			...
			for exp_name in exp_names
				@df aggregated_data[exp_name][agg] plot!(cols(col), label=exp_name, linewidth=1.0)
			end
...
for col in propertynames(single_run_data[exp_names[1]][1])
	if col != :step
		...
		for exp_name in exp_names
			c += 1
			@df single_run_data[exp_name][1] plot!(cols(col), color=palette(:default)[c], label=exp_name, linewidth=0.5)
			for i in 2:no_runs
				@df single_run_data[exp_name][i] plot!(cols(col), color=palette(:default)[c], label="", linewidth=0.5)
			end
		end
...
for exp_name in exp_names
	global c += 1
	for agg in run_aggregation
		if !isdir("$folder$exp_name/")
			mkdir("$folder$exp_name/")
		end
		if !isdir("$folder$exp_name/$agg/")
			mkdir("$folder$exp_name/$agg/")
		end
		...
		for col in propertynames(aggregated_data[exp_names[1]][agg])
			if col != :step
				...
				@df aggregated_data[exp_name][agg] plot!(cols(col), color=palette(:default)[c], label=exp_name, linewidth=1.0)

				savefig(pl, "$folder/$exp_name/$agg/$col.pdf")
			end
		end
	end
end
...
for exp_name in exp_names
	...
	for col in propertynames(single_run_data[exp_names[1]][1])
		if col != :step
			...
			savefig(pl, "$folder/$exp_name/$col.pdf")
		end
	end
end
